...
public enum DatabaseConfig {
    ...
    DATABASE_DBNAME("database.sql.dbname", "poixpixelcustom"),
    DATABASE_TABLEPREFIX("database.sql.table_prefix", "poixpixelcustom_"),
    ...
    DATABASE_POOLING_HEADER(
            ...
            "# Unless you actually know what you're doing and how PoixpixelCustom uses its mysql connection,",
    ...
    public static void loadDatabaseConfig(Path databaseConfigPath) {
        if (!FileMgmt.checkOrCreateFile(databaseConfigPath.toString())) {
            throw new PoixpixelCustomInitException("Failed to touch '" + databaseConfigPath + "'.", PoixpixelCustomInitException.PoixpixelCustomError.DATABASE_CONFIG);
        }
        // read the database.yml into memory
        databaseConfig = new CommentedConfiguration(databaseConfigPath);
        if (!databaseConfig.load())
            throw new PoixpixelCustomInitException("Database: Failed to load database.yml!", PoixpixelCustomInitException.PoixpixelCustomError.DATABASE_CONFIG);
    ...
    public static int getInt(DatabaseConfig node) {
        ...
        try {
            ...
            PoixpixelCustom.getPlugin().getLogger().severe(node.getRoot().toLowerCase() + " from database.yml");
            return 0;
        }
    }
